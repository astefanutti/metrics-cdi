...
public class ExceptionMeteredConstructorBeanTest {

    private final static String METER_NAME = MetricRegistry.name(ExceptionMeteredConstructorBean.class, ExceptionMeteredConstructorBean.class.getSimpleName(), ExceptionMetered.DEFAULT_NAME_SUFFIX);
    ...
    @Inject
    private Instance<ExceptionMeteredConstructorBean> instance;

    @Test
    @InSequence(1)
    public void exceptionMeteredConstructorNotCalledYet() {
        assertThat("Meter is not registered correctly", registry.getMeters().keySet(), is(empty()));
    }

    @Test
    @InSequence(2)
    public void exceptionMeteredConstructorCalled() {
